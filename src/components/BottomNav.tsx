...
export default function BottomNav() {
  ...
  if (activeIndex === -1) {
    activeIndex = tabs.findIndex(t => t.path !== '/' && currentPath.startsWith(t.path));
  }
      ...
      <div style={{ background: 'rgba(10,10,10,0.94)', backdropFilter: 'saturate(180%) blur(20px)', WebkitBackdropFilter: 'saturate(180%) blur(20px)', boxShadow: '0 -4px 30px rgba(0,0,0,0.15)', borderRadius: '24px 24px 0 0' }}>
        <div className="relative flex items-center max-w-md mx-auto">
          ...
          <div className="absolute top-1.5 bottom-1.5 rounded-2xl pointer-events-none"
            style={{ width: `${100 / tabs.length}%`, left: 0, transform: `translateX(${activeIndex * 100}%)`, background: 'rgba(255,255,255,0.1)', transition: 'transform 0.45s cubic-bezier(0.34, 1.56, 0.64, 1)' }}>
          ...
          {tabs.map((tab, i) => {
            ...
            return (
              <Link key={i} to={tab.path} className="relative z-10 flex-1 flex flex-col items-center justify-center gap-1 py-3.5">
